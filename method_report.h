...
struct MethodResult {
	...
	vector<vector<ScoreReport>> sr_list;

	void add_best_threshold(vector<ScoreReport> sr_list_for_same_method) {

		//vector<double> all_score_list;
		//for each (ScoreReport sr in sr_list_for_same_method) {
		//	for each (ImgScore is in sr.scoreList) {
		//		all_score_list.push_back(is.score);
		//	}
		//}
		//sort(all_score_list.rbegin, all_score_list.rend);
		...
		for each (ScoreReport sr in sr_list_for_same_method) { // for all score
			int correct = 0;
			if (sr.firstWrong != 0) {
				correct++;
			}
			for (int i = 0; i < sr.scoreList.size(); i++) { // for all score
	...
	void add_acc(vector<ScoreReport> sr_list) {
		...
		_acc_of.push_back(acc);
		_acc_first_match.push_back(acc_fm);
		_first_match_count.push_back(match_count);
	...
	void report_acc() {
		for (int i = 0; i < _acc_of.size(); i++) {
			...
			if (sr_list.size() > 0)
				for (int j = 0; j < sr_list[i].size(); j++) {
					sr_list[i][j].report();
					pair < double, double> pr = sr_list[i][j].getPRbyScore(best_threshold_fm[i]);
					printf("  - if threshold= %f , p= %3.4f%% , r= %3.4f%% \n", best_threshold_fm[i], pr.first, pr.second);
				}
